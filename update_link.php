...
if(!isset($_SESSION["id"])) {
    header('Location: https://linkema.com/97/loginregister-master/app/login.php');
}
...
if(isset($_POST["link_id"]) && !empty($_POST["link_id"])){
    ...
    $profile_id = $_POST["profile_id"];
        ...
        $stmt = $pdo->prepare("UPDATE links SET title=?,url=? WHERE link_id=?");

        if(!$stmt->execute([ $title,$url,$link_id  ])) {
	...
	$_GET["link_id"] = trim($_GET["link_id"]);
...
<head>
  ...
  <title>ویرایش لینک | لینک ما</title>
        ...
        <div class="row w-100">
          <div class="col-lg-4 mx-auto">
            <div class="auth-form-light text-right p-5">
                    ...
                    <p>در این قسمت می توانید این لینک را ویرایش کنید</p>
      ...
      $(function() {
         ...
         function check_url() {
            ...
            if (pattern.test(url) && url !== '') {
               ...
               $("#url_error_message").html("نشانی لینک نامعتبر است");
