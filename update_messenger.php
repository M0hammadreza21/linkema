...
if(isset($_POST["messenger_id"]) && !empty($_POST["messenger_id"])){
    ...
    $profile_id = $_POST["profile_id"];
        ...
        $stmt = $pdo->prepare("UPDATE messengers SET type=?,title=?,url=? WHERE messenger_id=?");

        if(!$stmt->execute([ $type,$title,$url,$messenger_id  ])) {
	...
	$_GET["messenger_id"] = trim($_GET["messenger_id"]);
    if(isset($_GET["messenger_id"]) && !empty($_GET["messenger_id"])){

        $messenger_id =  trim($_GET["messenger_id"]);


        $sql = "SELECT * FROM messengers WHERE messenger_id = ?";
        if($stmt = mysqli_prepare($link, $sql)){

            $param_id = $messenger_id;
...
<head>
  ...
  <title>ویرایش حساب شما در شبکه اجتماعی | لینک ما</title>
        ...
        <div class="row w-100">
          <div class="col-lg-4 mx-auto">
            <div class="auth-form-light text-right p-5">
                        ...
                        <h2>ویرایش حساب شبکه اجتماعی</h2>

                    <p>در این قسمت می توانید این حساب شبکه اجتماعی را ویرایش کنید</p>
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
                        
                        <div class="form-group">
                            <label for="exampleSelectGender">شبکه اجتماعی</label>
                            <select class="form-control" name="type" id="type">
                                <option value="whatsapp" <?php if($type==="whatsapp") {echo 'selected';} ?>>واتس اپ</option>
                                <option value="instagram" <?php if($type==="instagram") {echo 'selected';} ?>>اینستاگرام</option>
                                <option value="telegram" <?php if($type==="telegram") {echo 'selected';} ?>>تلگرام</option>
                            </select>
                        </div>
