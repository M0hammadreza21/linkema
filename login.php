...
if($_SERVER["REQUEST_METHOD"] == "POST"){
		...
		$sql = "SELECT * from user WHERE email LIKE '{$email}' AND password LIKE '{$password}' LIMIT 1";
    	$result = $mysqli->query($sql);
    	
    	$row = mysqli_fetch_assoc($result);
    	if (!$result->num_rows == 1) {
    		$error_message = "<p>نام کاربری یا رمزعبور اشتباه وارد شده است</p>";
    	} else {
    		$error_message = "<p>با موفقیت وارد شدید</p>";
...
<head>
  ...
  <title>ورود به سایت | لینک ما</title>
  ...
  <div class="container-scroller">
    <div class="container-fluid page-body-wrapper full-page-wrapper">
      <div class="content-wrapper d-flex align-items-center auth">
        <div class="row w-100">
          <div class="col-lg-4 mx-auto" id="login">
            <div class="auth-form-light text-right p-5">
                <h2 class="text-info text-center">ورود به سایت<i class="mdi mdi-lock-open-outline"></i></h2>
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
<label class="badge badge-gradient-danger"><?php if ($error_message !="") { echo $error_message;} ?></label>
<label class="badge badge-gradient-danger"><?php if ($_GET["w"] != "") { echo "ثبت نام با موفقیت انجام شد. برای ورود ، اطلاعات خود را وارد کنید";} ?></label>
                        ...
                        <div class="input-group">
				        	...
				        	<label class="form-check-label text-muted">
				        	  <input  type="checkbox" id="remember" name="remember" <?php if(isset($_COOKIE["loginId"])) { ?> checked <?php } ?>> مرا بیاد داشته باش
				        	</label>
			           	  </div>
				        </div>
                        <input type="submit" class="btn btn-block btn-gradient-primary btn-lg font-weight-medium auth-form-btn" value="ورود به سایت">
                    </form>
                    
                    <div class="text-center mt-4 font-weight-light">
                  تا حالا ثبت نام نکرده اید؟ <a href="https://linkema.com/register.php" class="text-primary">ثبت نام</a>
                </div>
                <div class="text-center mt-4 font-weight-light">
                <a href="https://linkema.com/reset_password.php" class="text-center mt-4 font-weight-light">رمزعبور را فراموش کرده اید؟</a>
                </div>
  ...
  <script type="text/javascript">
      $(function() {
        ...
        $("#login").bind('blur focus mouseenter', function() {
            check_password();
            check_validation();
         });
