...
if(isset($_POST["contact_id"]) && !empty($_POST["contact_id"])){
    ...
    $profile_id = $_POST["profile_id"];
        ...
        $stmt = $pdo->prepare("UPDATE contacts SET type=?,title=?,value=? WHERE contact_id=?");

        if(!$stmt->execute([ $type,$title,$value,$contact_id  ])) {
                echo "Something went wrong. Please try again later.";
                header("location: error.php");
            } else{
                $stmt = null;
                header("location: https://linkema.com/creator.php?profile_id=$profile_id");
            }
} else {
    
	$_GET["contact_id"] = trim($_GET["contact_id"]);
...
<head>
  ...
  <title>ویرایش اطلاعات تماس | لینک ما</title>
        ...
        <div class="row w-100">
          <div class="col-lg-4 mx-auto">
            <div class="auth-form-light text-right p-5">
                ...
                <h2>ویرایش اطلاعات تماس</h2>

                    <p>در این قسمت می توانید اطلاعات تماس خود را ویرایش کنید</p>
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
                        <div class="form-group">
                            <label for="exampleSelectGender">نوع</label>
                            <select class="form-control" name="type" dir="rtl" id="type">
                                <option value="tel" <?php if($type==="tel") {echo 'selected';} ?>>تلفن ثابت</option>
                                <option value="mobile" <?php if($type==="mobile") {echo 'selected';} ?>>تلفن همراه</option>
                                <option value="email" <?php if($type==="email") {echo 'selected';} ?>>ایمیل</option>
                                <option value="website" <?php if($type==="website") {echo 'selected';} ?>>وب سایت</option>
                            </select>

                        </div>
						<div class="form-group">
                            <label>عنوان</label>
                            <input type="text" name="title" id="title" dir="rtl" class="form-control" value="<?php echo $title; ?>">
                            <span class="badge badge-gradient-danger" id="title_error_message"></span>
                        </div>
						<div class="form-group">
                            <label><div id="myText"></div></label>
                            <input type="text" dir="ltr" name="value" id="value" class="form-control" value="<?php echo $value; ?>">
                            <span class="badge badge-gradient-danger" id="value_error_message"></span>
                        </div>
...
<script type="text/javascript">
      ...
      $(function() {
        ...
        function check_value() {
            ...
            if (value.length > 6) {
               $("#value_error_message").hide();
               $("#value").css("border-bottom","2px solid #34F458");
               error_value = false;
            } else {
               $("#value_error_message").html("نامعتبر است");
               $("#value_error_message").show();
               $("#value").css("border-bottom","2px solid #F90A0A");
               error_value = true;
            }
         }
...
$( "select" )
  .change(function () {
    ...
    switch(str) {
    case "tel":
      label_text = "شماره تلفن ثابت";
    break;
    case "mobile":
    label_text = "شماره تلفن همراه";
    break;
    case "email":
    label_text = "نشانی ایمیل";
