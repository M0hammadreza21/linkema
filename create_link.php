...
$sql = "SELECT MAX(position_order) as max_id FROM links";  
  ...
  $position_order = $row['max_id']+1;
        ...
        $stmt = $pdo->prepare("INSERT INTO links (pid,title,url,position_order) VALUES (?,?,?,?)"); 
        
        if($stmt->execute([ $profile_id,$title,$url,$position_order  ])) {
                $stmt = null;
                header("location: https://linkema.com/creator.php?profile_id=".$profile_id."&action=create_link");
            } else{
                echo "مشکلی پیش آمده";
            }
    }
          ...
          <div class="col-lg-4 mx-auto">
            <div class="auth-form-light text-right p-5">
                    <div class="page-header">
                        <h2 class="text-right">لینک جدید</h2>
                    </div>
                    <p>در این قسمت می توانید لینک جدید اضافه کنید</p>
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                        ...
                        <input type="submit" class="btn btn-primary" value="ثبت این لینک">
                    </form>
</div>
          </div>
        </div>
