            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="page-header clearfix">
                        <h2 class="float-left">لینک های وارد شده توسط کاربران </h2>
                    </div>

                    <div class="form-row">
                        <form action="links-index.php" method="get">
                        <div class="col">
                          <input type="text" class="form-control" placeholder="جستجو در میان لینک ها" name="search">
                        </div>
                    </div>
                        </form>
                    <br>
                    ...
                    $total_pages_sql = "SELECT COUNT(*) FROM links";
                    $result = mysqli_query($link,$total_pages_sql);
                    $total_rows = mysqli_fetch_array($result)[0];
                    $total_pages = ceil($total_rows / $no_of_records_per_page);
                    
                    //Column sorting on column name
                    $orderBy = array('pid', 'title', 'url'); 
                    $order = 'link_id';
                    ...
                    $sql = "SELECT * FROM links ORDER BY $order $sort LIMIT $offset, $no_of_records_per_page";
                    $count_pages = "SELECT * FROM links";

                    
                    if(!empty($_GET['search'])) {
                        $search = ($_GET['search']);
                        $sql = "SELECT * FROM links
                            WHERE CONCAT (block_id,title,url)
                            LIKE '%$search%'
                            ORDER BY $order $sort 
                            LIMIT $offset, $no_of_records_per_page";
                        $count_pages = "SELECT * FROM links
                            WHERE CONCAT (pid,title,url)
                            LIKE '%$search%'
                            ORDER BY $order $sort";
                    }
                    else {
                            ...
                            echo "<table class='table table-bordered table-striped'>";
                                echo "<thead>";
                                    echo "<tr>";
										...
										echo "<th><a href=?search=$search&sort=&order=url&sort=$sort>لینک</th>";
                                ...
                                while($row = mysqli_fetch_array($result)){
                                    echo "<tr>";
                                    echo "<td>" . $row['pid'] . "</td>";echo "<td>" . $row['title'] . "</td>";echo "<td>" . $row['url'] . "</td>";
                                        echo "<td>";
                                            echo "<a href='links-delete.php?link_id=". $row['link_id'] ."' data-toggle='tooltip'><i class='far fa-trash-alt'></i></a>";
                                        echo "</td>";
                                    echo "</tr>";
                                }
                                echo "</tbody>";
