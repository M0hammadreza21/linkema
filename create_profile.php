...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if (empty($_POST["title"]) || empty($_POST["subtitle"]) || empty($_POST["url"])) { $error_message = "لطفا همه فیلدها را پر کنید.";}
    else{
		...
		$sql = "SELECT * from profile WHERE url LIKE '{$url}' LIMIT 1";
        ...
        $stmt = $pdo->prepare("INSERT INTO profile (user_id,title,subtitle,url) VALUES (?,?,?,?)"); 
        
        if($stmt->execute([ $user_id,$title,$subtitle,$url  ])) {
                $stmt = null;
...
<head>
  ...
  <title>ایجاد صفحه جدید | لینک ما</title>
                ...
                <h1>ایجاد صفحه جدید</h1>
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                        <?php if (!empty($error_message)) {
                                echo "<label class='badge badge-gradient-danger'>".$error_message."</label>";
                        } ?>
						...
						<div class="form-group">
                            ...
                            <span class="badge badge-gradient-danger" id="subtitle_error_message"></span>
                        </div>
                        <div class="form-group">
                            <label>نشانی صفحه</label>
                            <div class="form-group">
                                <div class="input-group">
                                <div class="input-group-prepend">
                                    <input type="text" name="url" id="url" value="<?php echo $url; ?>" class="form-control">
                                <span class="input-group-text bg-gradient-primary text-white">https://linkema.com/</span>
                                </div>
                        ...
                        <input type="submit" id="submit" class="btn btn-primary" value="ایجاد صفحه">
                    </form>
      ...
      $(function() {
        ...
        function check_url() {
            ...
            if (url.length > 4) {
               ...
               $("#url_error_message").html("شناسه صفحه نامعتبر است. حداقل 4کاراکتر شامل حروف و اعداد");
        ...
        function check_title() {
            ...
            if (title.length > 6) {
               ...
               $("#title_error_message").html("شماره موبایل نامعتبر است");
        ...
        function check_subtitle() {
            ...
            if (subtitle.length > 6) {
               $("#subtitle_error_message").hide();
               $("#subtitle").css("border-bottom","2px solid #34F458");
               error_subtitle = false;
            } else {
               $("#subtitle_error_message").html("شماره موبایل نامعتبر است");
               $("#subtitle_error_message").show();
               $("#subtitle").css("border-bottom","2px solid #F90A0A");
               error_subtitle = true;
            }
         }

        function check_validation() {
        if (error_title === false && error_subtitle === false && error_url === false) {
